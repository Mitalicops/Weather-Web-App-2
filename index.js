...
function fetchWeatherData(location) {
  ...
  fetch(apiUrl)
    ...
    .then((data) => {
      //update today info
      const todayWeather = data.list[0].weather[0].description; // "weather" should be an array
 ...
 `;
      ...
      for (const dayData of nextDayData) {
        const forecastDate = new Date(dayData.dt_txt); // "day.Data.dt_txt" should be "dayData.dt_txt"
      ...
      `;
        ...
        //stop after getting a distinct days
        if (count === 4) break;
      }
    })
    .catch((error) => {
      alert(`Error fetching weather data: ${error.message} (Api Error)`); // Use backticks for template literals
    });
}
